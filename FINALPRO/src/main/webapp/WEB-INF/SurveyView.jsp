...
function change(el)
{
	...
	$.ajax({
		...
		{
			window.location="/Survey/Question1";
		}
		})
...
function changeStatus(el)
{
	...
	var SurveyStatusObjAsJSON=JSON.stringify(SurveyStatusObj);
	alert(SurveyStatusObjAsJSON)
	
	
	$.ajax({
		url:"/Survey/Status",
		type:"POST",
		data:SurveyStatusObjAsJSON,
		contentType:"application/json",
		success:function(response,status)
		{
			alert(response)
	...
	if(status=="false")
		...
		alert(SurveyShareObjAsJSON)
		 ...
		 $.ajax({
			...
			{
				alert(response)
				window.location="/Survey/sharesurvey";
			}
			}) 
		}
	...
	<div id="three-column" class="container">
		<div class="title">
		 ...
		 <form>
					...
					<td><input type="button" value="Share Survey" name="GO"  onclick="ShareSurvey(this);" id=<%=f.getSid()+"_share"%> class="btn"/></td>
